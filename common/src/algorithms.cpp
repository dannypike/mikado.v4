...
namespace mikado::common {
   ...
   ///////////////////////////////////////////////////////////////////////
   //
   bool poGetBool(po::variables_map const &cfg, string const &propertyName
      , bool const *defaultValue, source_location const &loc) {

      try {
         return cfg.at(propertyName).as<bool>();
      }
      catch (const std::exception &e) {
         // If there is no default value, then we will log an error
         if (!defaultValue) {
            log_exception(e);
            MikadoLog::MikadoLogger.streamError(loc) << "configuration property '"
               << propertyName << "' is not a bool value" << endl;
            if (cfg.count(propertyName)) {
               str_error() << "... the property exists, but has value '"
                  << cfg.at(propertyName).as<string>() << "', which cannot be converted to a bool" << endl;
            }
         }
      }
   ...
   ///////////////////////////////////////////////////////////////////////
   //
   unsigned poGetUnsigned(po::variables_map const &cfg, string const &propertyName
      , unsigned const *defaultValue, source_location const &loc) {

      try {
         return (unsigned)cfg.at(propertyName).as<unsigned>();
      }
      catch (const std::exception &e) {
         // If there is no default value, then we will log an error
         if (!defaultValue) {
            log_exception(e);
            MikadoLog::MikadoLogger.streamError(loc) << "configuration property '"
               << propertyName << "' is not an unsigned value" << endl;
            if (cfg.count(propertyName)) {
               str_error() << "... the property exists, but has value '"
                  << cfg.at(propertyName).as<string>() << "', which cannot be converted to an unsigned" << endl;
            }
         }
      }
   ...
   ///////////////////////////////////////////////////////////////////////
   //
   float poGetFloat(po::variables_map const &cfg, string const &propertyName
      , float const *defaultValue, source_location const &loc) {

      try {
         return (float)cfg.at(propertyName).as<float>();
      }
      catch (const std::exception &e) {
         // If there is no default value, then we will log an error
         if (!defaultValue) {
            log_exception(e);
            MikadoLog::MikadoLogger.streamError(loc) << "configuration property '"
               << propertyName << "' is not an float value" << endl;
            if (cfg.count(propertyName)) {
               str_error() << "... the property exists, but has value '"
                  << cfg.at(propertyName).as<string>() << "', which cannot be converted to an float" << endl;
            }
         }
      }
      return defaultValue ? *defaultValue : 0;
