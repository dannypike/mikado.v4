...
namespace mikado::torchBox {
   ...
   MikadoErrorCode NetMakeMore::createTrainingTensors() {
      try
      {
         ...
         if (auto deviceType = getC10Device(); deviceType != c10::DeviceType::CPU) {
            str_info() << "transferring tensors to " << deviceType << endl;
            for (int ii = 0; ii < (int)Subset::SubsetCount; ++ii) {
               ...
               str_info() << "tensor[" << (Subset)ii << "] is at " << tensors_[ii].device() << endl
                  << "    data = [" << toStringFlat<int64_t>(tensors_[(int)Subset::TrainX], 32) << "...]" << endl
                  ;
            }
         }
#endif
   ...
   ///////////////////////////////////////////////////////////////////////////
   //
   MikadoErrorCode NetMakeMore::buildTrainingData(WordIter itBeginWords, WordIter itEndWords) {
      ...
      try
      {
         // MakeMore predicts the next character in a name, so we train on each successive character 
         // in each name. We want the network to build a concept of "what letter comes after these letters?"
         while (itBeginWords < itEndWords) {

            // The character that we want the network to predict during training
            vocab_t ix;

            // The context that the network will use to predict the next letter, padded
            // to the context size with kSeparator characters.
            list<vocab_t> context(contextSize_, stoi_[kSeparator]);

            string word = *itBeginWords++ + kSeparator;
            for (auto ch : word) {
               // xx_ is the training input data (the current context)
               xx_.insert(xx_.end(), context.begin(), context.end());

               // yy_ is the expected output value given that context
               ix = stoi_[ch];
               yy_.push_back(ix);

               // Add the new letter into the context and go around again
               context.push_back(ix);
   ...
   MikadoErrorCode NetMakeMore::reportLoss(Subset subsetX, Subset subsetY) {
      ...
      SplitLossType splitLoss
         ...
         str_info() << "loss = " << loss.item<float>() << endl;
         };
